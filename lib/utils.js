...
function createStreamThroughputReporter(name) {
  ...
  let statsReporter = () => {
    ...
    console.log(`STATS[${name}]: rate=${rate}, total=${total}, ema=${ema.toFixed(2)}, avg=${(total / ticks).toFixed(2)}`)
    rate = 0
  }

  let statsReporterInterval = setInterval(statsReporter, 1000)
  return stream =>
    stream
      .consume(function (err, val, push, next) {
        if (err) {
          push(err)
          next()
        } else if (val === hi.nil) {
...
module.exports = { createStreamThroughputReporter, createContinuousRateLimiter }