...
async function batchWriteItems(batch, ddb) {
  ...
  do {
    ...
    const res = await ddb.send(cmd).catch((err) => {
      if (!err.retryable) throw err;
      console.error(`WARNING: DynamoDB call failed. batch=${batchId}, attempt=${attempt}, err=${JSON.stringify(err)}`);
...
function createStreamWriter(table, ratelimit, opts = {}) {
  ...
  return (stream) =>
    stream
      ...
      .batch(25)
      .map(
        hi.wrapCallback(function (batch, callback) {
...
async function fillTable(table, numItems, rate, partitionKey) {
  ...
  return hi(function (push, next) {
    push(null, {
      [partitionKey]: { S: (item++).toString() },
      value: { S: crypto.randomBytes(8).toString("hex") },
    });
